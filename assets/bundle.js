	...
	function todos() {
		...
		switch (action.type) {
			...
			case _actions.COMPLETE_TOTO:
				return [].concat(_toConsumableArray(state.slice(0, action.index)), [Object.assign({}, state[action.index], { completed: !state[action.index].completed })], _toConsumableArray(state.slice(action.index + 1)));
			default:
				return state;
		}
	}
		...
		_createClass(App, [{
			...
			value: function render() {
				...
				return _react2.default.createElement(
					...
					_react2.default.createElement(_TodoList2.default, { todos: this.props.todos, onTodoClick: function onTodoClick(index) {
